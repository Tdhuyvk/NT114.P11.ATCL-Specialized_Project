...
def simulate_raft(nodes):
    ...
    time.sleep(16)  # Chạy mô phỏng trong 16 giây
    for node in nodes:
        node.alive = False
...
def export_to_excel(nodes):
    ...
    df.to_excel("raft_metrics_7_node.xlsx", index=False)
    print("Kết quả đã được lưu vào file 'raft_metrics_7_node.xlsx'.")

if __name__ == "__main__":
    # Tạo 7 node
    nodes = [Node(node_id=i) for i in range(7)]
    simulate_raft(nodes)
