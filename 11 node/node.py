...
def simulate_raft(nodes):
    ...
    time.sleep(9.5325)  # Chạy mô phỏng trong 9.5325 giây
    for node in nodes:
        node.alive = False
...
def export_to_excel(nodes):
    ...
    df.to_excel("raft_metrics_11_node.xlsx", index=False)
    print("Kết quả đã được lưu vào file 'raft_metrics_11_node.xlsx'.")

if __name__ == "__main__":
    # Tạo 11 node
    nodes = [Node(node_id=i) for i in range(11)]
    simulate_raft(nodes)
